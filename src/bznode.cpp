...
/*    bool fRequireUpdate = nProtocolVersion < mnpayments.GetMinBznodePaymentsProto() ||
    ...
    bool fRequireUpdate = nProtocolVersion < mnpayments.GetMinBznodePaymentsProto() ||
                          // or it's our own node and we just updated it to the new protocol but we are still waiting for activation ...
                          (fOurBznode && (nProtocolVersion < MIN_BZNODE_PAYMENT_PROTO_VERSION_1 || nProtocolVersion > MIN_BZNODE_PAYMENT_PROTO_VERSION_2));
...
//        LogPrintf("mnpayments.mapBznodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2)=%s\n", mnpayments.mapBznodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2));
        if (mnpayments.mapBznodeBlocks.count(BlockReading->nHeight) &&
            ...
            CAmount nBznodePayment = GetBznodePayment(BlockReading->nHeight, block.vtx[0].GetValueOut());
...
bool CBznodeBroadcast::CheckOutpoint(int &nDos) {
...

    {
        ...
        if (coins.vout[vin.prevout.n].nValue != BZNODE_COIN_REQUIRED * COIN) {
            LogPrint("bznode", "CBznodeBroadcast::CheckOutpoint -- Bznode UTXO should have 2000 BZX, bznode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 15000 BZX tx got nBznodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex *pMNIndex = (*mi).second; // block for 15000 BZX tx -> 1 confirmation
