...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fBZNode) {
            LogPrintf("DSACCEPT -- not a Bznode!\n");
        ...
        CBznode *pmn = mnodeman.Find(activeBznode.vin);
        if (pmn == NULL) {
        ...
        CBznode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeyBznode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToBznode) return;
            if ((CNetAddr) pSubmittedToBznode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Bznode: pSubmittedToBznode=%s, addr=%s\n", pSubmittedToBznode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if (nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on bznode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Bznode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Bznode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from bznode %s\n", dsq.ToString(), pmn->addr.ToString());
            if (pSubmittedToBznode && pSubmittedToBznode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if (fBZNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Bznode!\n");
            return;
        }

        if (!pSubmittedToBznode) return;
        if ((CNetAddr) pSubmittedToBznode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Bznode: pSubmittedToBznode %s addr %s\n", pSubmittedToBznode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fBZNode) {
            LogPrintf("DSSIGNFINALTX -- not a Bznode!\n");
            return;
        }
        ...
        if (fBZNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Bznode!\n");
            return;
        }

        if (!pSubmittedToBznode) return;
        if ((CNetAddr) pSubmittedToBznode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Bznode: pSubmittedToBznode %s addr %s\n", pSubmittedToBznode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fBZNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Bznode!\n");
            return;
        }

        if (!pSubmittedToBznode) return;
        if ((CNetAddr) pSubmittedToBznode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Bznode: pSubmittedToBznode=%s  addr=%s\n", pSubmittedToBznode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus() {
    ...
    if ((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !bznodeSync.IsBlockchainSynced())
    ...
    switch (nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to bznode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if (nEntriesCount == 0) {
                ...
                if (nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to bznode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if (nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if (nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if (nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to bznode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
        case POOL_STATE_SIGNING:
...
//
// Check the mixing progress and send client updates if a Bznode
//
...
void CDarksendPool::CommitFinalTransaction() {
    if (!fBZNode) return; // check and relay final tx only on bznode
    ...
    // create and sign bznode dstx transaction
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeBznode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to Bznodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Bznode
...
void CDarksendPool::CheckTimeout() {
    ...
    if (!fEnablePrivateSend && !fBZNode) return;
    ...
    int nLagTime = fBZNode ? 0 : 10000; // if we're the client, give the server a few extra seconds before resetting.
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeBznode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a Bznode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fBZNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Bznode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a Bznode
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Bznode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from Bznode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("Bznode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Bznode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    // push all of our signatures to the Bznode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the bznode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    if (!fEnablePrivateSend || fBZNode || !pCurrentBlockIndex) return false;
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Bznodes detected\n");
        strAutoDenomResult = _("No Bznodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the Bznode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecBznodesUsed: size: %d, threshold: %d\n", (int) vecBznodesUsed.size(), nThreshold_high);

    if ((int) vecBznodesUsed.size() > nThreshold_high) {
        vecBznodesUsed.erase(vecBznodesUsed.begin(), vecBznodesUsed.begin() + vecBznodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecBznodesUsed: new size: %d, threshold: %d\n", (int) vecBznodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CBznode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq bznode is not in bznode list, bznode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecBznodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to bznode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Bznode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fBznode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToBznode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Bznode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CBznode *pmn = mnodeman.FindRandomNotInVec(vecBznodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random bznode!\n");
            strAutoDenomResult = _("Can't find random Bznode.");
            return false;
        }
        vecBznodesUsed.push_back(pmn->vin);

        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this bznode!"
                              " bznode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Bznode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fBznode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
    ...
    strAutoDenomResult = _("No compatible Bznode found.");
    return false;
}
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fBZNode || nSessionID != 0) return false;
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeBznode.vin, GetTime(), false);
...
bool CDarksendPool::AddUserToExistingSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fBZNode || nSessionID == 0 || IsSessionReady()) return false;
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:
            return _("Not in the Bznode list.");
        case ERR_MODE:
            return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:
            return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:
            return _("This is not a Bznode.");
        case ERR_QUEUE_FULL:
            return _("Bznode queue is full.");
        case ERR_RECENT:
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == BZNODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeBznode.pubKeyBznode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyBznode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeyBznode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Bznode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeyBznode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToBznode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fBZNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Bznode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && bznodeSync.IsBznodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    ...
    while (true) {
        ...
        bznodeSync.ProcessTick();

        if (bznodeSync.IsBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all bznodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if (nTick % BZNODE_MIN_MNP_SECONDS == 15)
                activeBznode.ManageState();

            if (nTick % 60 == 0) {
                mnodeman.ProcessBznodeConnections();
                mnodeman.CheckAndRemove();
                mnpayments.CheckAndRemove();
                instantsend.CheckAndRemove();
            }
            if (fBZNode && (nTick % (60 * 5) == 0)) {
