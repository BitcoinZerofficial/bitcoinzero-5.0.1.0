...
bool CBznodeSync::IsBlockchainSynced(bool fBlockAccepted) {
    ...
    LogPrint("bznode-sync", "CBznodeSync::IsBlockchainSynced -- state before check: %ssynced, skipped %d times\n", fBlockchainSynced ? "" : "not ", nSkipped);
...
void CBznodeSync::ProcessTick() {
    ...
    if (nRequestedBznodeAssets == BZNODE_SYNC_INITIAL && IsBlockchainSynced()) {
        SwitchToNextAsset();
    }
