...
void CActiveBznode::ManageStateInitial() {
    ...
    if (pwalletMain->GetBalance() < BZNODE_COIN_REQUIRED * COIN) {
        LogPrintf("CActiveBznode::ManageStateInitial -- %s: Wallet balance is < 15000 BZX\n", GetStateString());
        return;
    }
...
void CActiveBznode::ManageStateRemote() {
    ...
    if (infoMn.fInfoValid) {
        if (infoMn.nProtocolVersion < MIN_BZNODE_PAYMENT_PROTO_VERSION_1
                || infoMn.nProtocolVersion > MIN_BZNODE_PAYMENT_PROTO_VERSION_2) {
            nState = ACTIVE_BZNODE_NOT_CAPABLE;
