...
public:
    TxViewDelegate(const PlatformStyle *platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(BitcoinUnits::BZX),
...
void OverviewPage::handleEnabledTorChanged(){
	...
	if(ui->checkboxEnabledTor->isChecked()){
		if (WriteBinaryFileTor(pathTorSetting.string().c_str(), "1")) {
			msgBox.setText("Please restart the Bitcoinzero wallet to route your connection to TOR to protect your IP address. \nSyncing your wallet might be slower with TOR.");
		} else {
			msgBox.setText("Anonymous communication cannot enable");
		}
	}else{
		if (WriteBinaryFileTor(pathTorSetting.string().c_str(), "0")) {
			msgBox.setText("Please restart the Bitcoinzero wallet to disable route your connection to TOR to protect your IP address.");
		} else {
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("BZX")
    updateDisplayUnit();
}
